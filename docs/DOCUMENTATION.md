...
The `require_role("admin")`/`require_role("admin", "staff")` dependency (`backend/core/security/rbac.py`) checks `current_user.role` (or `current_user.is_superuser`) against the given role names, and is wired into every user-listing and product-write route. **Correction to a previous revision of this document:** a separate, finer-grained `require_permission(*permission_keys)` dependency does exist in the same module, checking `current_user.permissions` (or `is_superuser`) against a permission key rather than a role name — it's what gates every `billing.manage` route in §4/§14 and the RBAC-management routes themselves. There genuinely is no `AuthorizationPolicy`/`require_policy`-based route in the current code (`require_policy` exists in `rbac.py` but nothing calls it), but the RBAC admin surface is real: `POST /api/v1/admin/roles`, `POST /api/v1/admin/permissions`, `PUT /api/v1/admin/roles/{role_id}/permissions`, `POST /api/v1/admin/users/{user_id}/roles`, and `GET /api/v1/rbac/check-permission` (all in `backend/app/api/v1/rbac/router.py`, mounted with no additional prefix), gated by `require_permission("rbac.manage")`. These were missing from earlier revisions of this table; noted here rather than fully expanded, since the primary gap this revision addresses is the billing/subscription module (§14).
...
- **CI: exists at `.github/workflows/ci.yml`**, running on push/PR to `main`/`master`: lint (`ruff check backend tests`) → real `alembic upgrade head` against a Postgres service container → `pytest -q` → `scripts/security_scan.sh`. `aiosqlite` is now declared in `pyproject.toml`'s `dev` extra, so the test step's dependency is satisfied out of the box. The **lint step still fails** as of this revision — `ruff check backend tests` reports 12 errors (down from a previously-reported 24), all confined to `tests/` (unused `pytest` imports and a `conftest.client` fixture re-imported and then redefined in a handful of test modules), 5 of which `ruff check backend tests --fix` resolves automatically. This would still block the later CI steps from running, but it's a test-file cleanup, not an application-code or dependency problem.
...
- **CI lint step currently fails:** `ruff check backend tests` — the first step of `.github/workflows/ci.yml` — reports 12 errors (down from a previously-reported 24) as shipped, which blocks the rest of the pipeline (migrations-against-Postgres, tests, security scan) from ever running in CI. All 12 are confined to `tests/`: unused `pytest` imports in a few files, and a `client` fixture imported from `conftest` then redefined by a same-named test function in `tests/test_authorization_and_roles.py`. 5 of the 12 are auto-fixable with `ruff check backend tests --fix`; the rest need the redefined `client` parameter renamed.
...
- **`aiosqlite` is declared as a dependency:** `pyproject.toml`'s `dev` extra now includes `aiosqlite>=0.19.0` alongside `pytest`/`ruff`/`mypy`. A fresh `pip install -e .[dev] && pytest -q` runs the full suite without a manual `pip install aiosqlite` step.
...
- **Get the CI pipeline green** — clean up the 12 remaining `ruff` errors in `tests/` (`ruff check backend tests --fix` handles 5 of them automatically; the rest need the redefined `client` fixture parameter renamed in `tests/test_authorization_and_roles.py`). Neither the app nor the migrations are broken; the pipeline just can't currently prove that end-to-end because of this lint step.
