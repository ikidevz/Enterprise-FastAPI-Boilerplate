...
- Use-case classes now live under feature-oriented packages such as backend/application/users, backend/application/products, and backend/application/auth so the application layer is organized the same way as the domain layer.
...
3. Add application behavior.
   - Define one or more application use cases under feature-focused packages such as backend/application/users, backend/application/products, or backend/application/auth to capture the workflow explicitly.
...
- backend/application/use_cases.py or a feature-specific use case module
...
This pattern keeps the router thin and ensures most logic lives in the service, application, and infrastructure layers rather than being embedded directly in the API surface.

## 4. How the current features work
...
- Login, token creation, and lockout logic are coordinated in backend/application/auth/use_cases.py.
...
- Product creation and update now use application use cases in backend/application/products/use_cases.py so the route layer remains thin and the workflow is easy to test.
...
- backend/application/users/use_cases.py, backend/application/products/use_cases.py, and backend/application/auth/use_cases.py: feature-specific use-case modules for users, products, and authentication
...
- backend/application contains orchestration and feature-organized use-case entry points
